			...
			<div className="space-y-10 py-[calc(50vh-100px)] text-white/60 hidden md:block">
				...
				<Snapper>
					<p className="text-xl leading-8 text-dirt selection:bg-dirt selection:text-midnight">
						Building products with TypeScript for {experience}+ years, with a
						strong passion for UX and interaction design.
					</p>
				</Snapper>
				<Snapper>
					<Socials />
...
function Snapper({ children }: React.PropsWithChildren) {
	return (
		<div className="size-50 flex items-center justify-center snap-always snap-center ring-0 ring-dirt">
			<div className="absolute left-32 max-w-105">{children}</div>
		</div>
	);
}
